<p align="center">
  <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/banner.svg?v=4" alt="Hamza Khan - Backend-focused engineer" width="100%" />
</p>

<p align="center">
  <a href="https://hamzakhan.dev"><strong>hamzakhan.dev</strong></a>
  ·
  <a href="https://drive.google.com/drive/folders/1FhPuvKIdNflIOokBWOcP1ska-v2QXM4f"><strong>resume</strong></a>
  · Karachi, Pakistan
  ·
  <a href="mailto:hamzaskhaan@gmail.com">email</a>
</p>

<p align="center">
  <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/sparks-divider.svg?v=4" alt="" width="100%" />
</p>

Backend-focused full stack engineer. Production work in distributed systems, AI infrastructure, and cloud-native backends (Python & Node.js).

Currently: sole backend on **PitchAwesome** (FastAPI · Mongo · Redis · AWS). Also robotics (Isaac Sim / Lab) and AI pipelines.

## Weekly summary | What we achieved

Gemini writes this from my notes and public work, once a week.

<!-- WEEK:START -->
### Week of 02 Aug 2026

**Notes touched**
- [engineering-notes] Add draft engineering notes — README.md, ai/agents.md, ai/embeddings.md, ai/rag.md, backend/jwt.md, backend/mongodb-transactions.md
- [engineering-notes] Initial commit — LICENSE, README.md
- [Research-Notes] Add personal reading notes — README.md, notes/attention.md, notes/react.md, topics/memory.md
- [Research-Notes] Initial commit — LICENSE, README.md
- [Architectural-Systems] Add Mermaid architecture sketches — README.md, fastapi-authentication.md, mongo-replica-set.md, nestjs-modules.md, redis-cache.md, robot-control-loop.md
- [Architectural-Systems] Initial commit — LICENSE, README.md
- [How-I-solved-it] Add placeholder problem write-ups — README.md, cases/auth-boundary-refactor/README.md, cases/motor-event-loop-blocking/README.md
- [How-I-solved-it] Initial commit — LICENSE, README.md

Next:
Add one production note worth summarizing next week.
<!-- WEEK:END -->

<p align="center">
  <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/sparks-divider.svg?v=4" alt="" width="100%" />
</p>

## Stack

<p align="center">
  <img src="https://img.shields.io/badge/Python-0B1410?style=for-the-badge&logo=python&logoColor=4ADE80" alt="Python" />
  <img src="https://img.shields.io/badge/FastAPI-0B1410?style=for-the-badge&logo=fastapi&logoColor=F87171" alt="FastAPI" />
</p>
<p align="center">
  <img src="https://img.shields.io/badge/JavaScript-0B1410?style=for-the-badge&logo=javascript&logoColor=4ADE80" alt="JavaScript" />
  <img src="https://img.shields.io/badge/TypeScript-0B1410?style=for-the-badge&logo=typescript&logoColor=F87171" alt="TypeScript" />
  <img src="https://img.shields.io/badge/Node.js-0B1410?style=for-the-badge&logo=nodedotjs&logoColor=4ADE80" alt="Node.js" />
  <img src="https://img.shields.io/badge/NestJS-0B1410?style=for-the-badge&logo=nestjs&logoColor=F87171" alt="NestJS" />
</p>
<p align="center">
  <img src="https://img.shields.io/badge/React-0B1410?style=for-the-badge&logo=react&logoColor=4ADE80" alt="React" />
  <img src="https://img.shields.io/badge/Vue-0B1410?style=for-the-badge&logo=vuedotjs&logoColor=F87171" alt="Vue" />
  <img src="https://img.shields.io/badge/Tailwind-0B1410?style=for-the-badge&logo=tailwindcss&logoColor=4ADE80" alt="Tailwind" />
</p>
<br/>
<p align="center">
  <img src="https://img.shields.io/badge/MongoDB-0B1410?style=for-the-badge&logo=mongodb&logoColor=4ADE80" alt="MongoDB" />
  <img src="https://img.shields.io/badge/PostgreSQL-0B1410?style=for-the-badge&logo=postgresql&logoColor=F87171" alt="PostgreSQL" />
  <img src="https://img.shields.io/badge/Redis-0B1410?style=for-the-badge&logo=redis&logoColor=4ADE80" alt="Redis" />
  <img src="https://img.shields.io/badge/Docker-0B1410?style=for-the-badge&logo=docker&logoColor=F87171" alt="Docker" />
</p>
<p align="center">
  <img src="https://img.shields.io/badge/AWS-0B1410?style=for-the-badge&logo=amazonwebservices&logoColor=4ADE80" alt="AWS" />
  <img src="https://img.shields.io/badge/GCP-0B1410?style=for-the-badge&logo=googlecloud&logoColor=F87171" alt="GCP" />
  <img src="https://img.shields.io/badge/Firebase-0B1410?style=for-the-badge&logo=firebase&logoColor=4ADE80" alt="Firebase" />
</p>
<br/>
<p align="center">
  <img src="https://img.shields.io/badge/Isaac%20Sim-0B1410?style=for-the-badge&logo=nvidia&logoColor=F87171" alt="Isaac Sim" />
  <img src="https://img.shields.io/badge/Isaac%20Lab-0B1410?style=for-the-badge&logo=nvidia&logoColor=4ADE80" alt="Isaac Lab" />
  <img src="https://img.shields.io/badge/Ubuntu-0B1410?style=for-the-badge&logo=ubuntu&logoColor=F87171" alt="Ubuntu" />
</p>

<p align="center">
  <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/sparks-divider.svg?v=4" alt="" width="100%" />
</p>

## Chain of thought

A resume lists career outcomes. These repos are the notes that document the chain of thought behind them — the path from A to B. What the resume highlights, these explain in depth.

<table width="100%">
  <tr>
    <td width="50%" valign="top">
      <a href="https://github.com/hamzaskhan/engineering-notes">
        <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/window-engineering.svg?v=4" alt="engineering-notes" width="100%" />
      </a>
    </td>
    <td width="50%" valign="top">
      <a href="https://github.com/hamzaskhan/Research-Notes">
        <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/window-research.svg?v=4" alt="Research-Notes" width="100%" />
      </a>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <a href="https://github.com/hamzaskhan/Architectural-Systems">
        <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/window-architecture.svg?v=4" alt="Architectural-Systems" width="100%" />
      </a>
    </td>
    <td width="50%" valign="top">
      <a href="https://github.com/hamzaskhan/How-I-solved-it">
        <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/window-solved.svg?v=4" alt="How-I-solved-it" width="100%" />
      </a>
    </td>
  </tr>
</table>

<p align="center">
  <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/sparks-divider.svg?v=4" alt="" width="100%" />
</p>

## Work signature

<p align="center">
  <img src="https://raw.githubusercontent.com/hamzaskhan/hamzaskhan/main/assets/insight-graph.svg?v=3" alt="Work signature insight graph" width="100%" />
</p>
